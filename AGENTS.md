...
To save a managed value from a private terminal, run `task secrets:set -- NAME` and enter it at the hidden prompt. In a CAO execute-capable work session, call Agent MCP `request_managed_secret_set` with the declared name; the operator supplies the value through the dashboard's private one-shot form, and the agent checks completion with `managed_secret_set_status`. Never send the value through an agent tool argument, terminal answer, message, shell text, argument, environment assignment, or file.
